...
def request(index):
	...
	while not ex.is_set():
		...
		try:
			...
			pass
		if err_count >= 20:
			print("\033[92mProxy\033[0m " + ips[index] + " \033[94mHiBerApi Attack")
			return
...
def cloudFlareCheck():
	...
	if isCloudFlare(url) is True:
		...
		for i in range(5, 0, -1):
			...
			time.sleep(1)
		print('\n\033[92mStart Attack...\033[0m')
		startAttack()
	else:
		print('\033[92mStart Attack...\033[0m')
		startAttack()
...
def showUsage():
	print("Usage: HiBerApi.py [-v] <http-link> [-a] <login:pass> [-t] <timeout> HiBerApi Test By:xHiz\n")
